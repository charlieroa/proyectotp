...
class OrderController extends GetxController {
  ...
  Future<void> initMapAssets() async {
    List<LatLng> points = [
    ...
    polylines.addAll([Polyline(polylineId: PolylineId("1"), points: points, width: 2, color: theme.colorScheme.primary)]);

    deliveryPinIcon = BitmapDescriptor.bytes(await getBytesFromAsset(ShoppingCache.deliveryIcon, 56));
    shopPinIcon = BitmapDescriptor.bytes(await getBytesFromAsset(ShoppingCache.shopIcon, 100));
    deliveryBoyPinIcon = BitmapDescriptor.bytes(await getBytesFromAsset(ShoppingCache.deliveryBoyIcon, 64));
    markers.addAll([
      Marker(
  ...
  Future<void> onMapCreated(GoogleMapController controller) async {
    ...
    mapStyle = await rootBundle.loadString(AppTheme.themeType == ThemeType.light ? ShoppingCache.lightMap : ShoppingCache.darkMap);
    update();
  }
