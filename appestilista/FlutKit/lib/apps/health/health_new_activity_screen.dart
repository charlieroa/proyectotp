...
class HealthNewActivityScreen extends StatefulWidget {
  const HealthNewActivityScreen({super.key});

  @override
  State<HealthNewActivityScreen> createState() => _HealthNewActivityScreenState();
}
...
class _HealthNewActivityScreenState extends State<HealthNewActivityScreen> {
    ...
    return Scaffold(
      appBar: AppBar(
        ...
        leading: InkWell(
          ...
          child: Icon(LucideIcons.chevron_left, color: theme.colorScheme.onSurface, size: 20),
        ),
      ),
      body: ListView(
          ...
          RichText(
            ...
            text: TextSpan(
              children: <TextSpan>[
                TextSpan(
                  text: "What's your",
                  style: MyTextStyle.headlineSmall(color: theme.colorScheme.onSurface, fontWeight: 600),
                ),
                TextSpan(
                  text: " goal?",
                  style: MyTextStyle.headlineSmall(color: customTheme.colorSuccess, fontWeight: 600),
                ),
              ],
            ),
          ),
  ...
  Widget singleChoice({required String title}) {
    return MyContainer(
      ...
      borderRadiusAll: 4,
      child: MyText.bodyLarge(title, color: theme.colorScheme.onSurface, fontWeight: 500),
    );
  }
}
