...
// Limpieza de texto para busquedas publicas (trim + colapso espacios)
...
/* ===========================================
   Resolver tenant desde numero receptor (WA)
=========================================== */
async function resolveTenantIdFromPhone(req) {
  ...
  if (phoneNumberId) {
    const t = await prisma.tenant_numbers.findUnique({
      where: { phone_number_id: phoneNumberId },
      select: { tenant_id: true },
    });
    mappingTenantId = t?.tenant_id || null;
  }

  if (!mappingTenantId && tenantId && phoneNumberId) {
    await prisma.tenant_numbers.upsert({
      where: { phone_number_id: phoneNumberId },
      update: {
        tenant_id: tenantId,
        provider: 'whatsapp',
...
/* ===========================================
   Validacion: horario de estilista dentro del tenant
=========================================== */
const validateStylistWorkingHoursAgainstTenant = async (tenantId, stylistHours) => {
  ...
  for (const day of tenantDays) {
    ...
    if (sDay?.active && !normalizedTDay?.active) {
      throw new Error(`El estilista no puede trabajar el ${day} si el salon esta cerrado.`);
    }
    ...
    if (sDay?.active && commonRanges.length === 0) {
      throw new Error(`El horario del estilista el ${day} no coincide con el horario del salon.`);
    }
  }
};
...
exports.searchStylists = async (req, res) => {
  ...
  if (!query)   return res.status(400).json({ error: 'Falta el parametro de busqueda (query)' });
  ...
  try {
    const sql = `
      ...
      LIMIT 10
    `;
    const result = await prisma.$queryRawUnsafe(sql, tenantId, like, exact, starts);

    return res.status(200).json({
      stylists: result.map(s => ({
        id: s.id,
  ...
  try {
    ...
    if (working_hours !== undefined && working_hours !== null) {
      try {
        ...
        return res.status(400).json({ error: e.message || 'working_hours invalido' });
      }
    }
    ...
    if (isClient) {
      ...
      return res.status(201).json(dbToApiUser(newUser));
    }
...
exports.getAllUsersByTenant = async (req, res) => {
  ...
  try {
    ...
    return res.status(200).json(users.map(dbToApiUser));
  } catch (error) {
...
exports.getUserById = async (req, res) => {
  ...
  try {
    ...
    return res.status(200).json(dbToApiUser(user));
  } catch (error) {
...
exports.updateUser = async (req, res) => {
  ...
  try {
    const existingUser = await prisma.users.findUnique({
      where: { id },
      select: { tenant_id: true, role_id: true },
    });
    if (!existingUser) {
      return res.status(404).json({ message: 'Usuario no encontrado' });
    }
    const { tenant_id, role_id } = existingUser;
    const isStylist = parseInt(role_id, 10) === 3;
    const isClient  = parseInt(role_id, 10) === 4;
    ...
    if (Object.keys(data).length === 0) {
      ...
      if (isClient) {
        const dbUser = dbToApiUser(currentUser);
        return res.status(200).json({
          id: dbUser.id,
        ...
        return res.status(200).json(dbToApiUser(currentUser));
      }
    }
    ...
    if (isClient) {
      const updatedDbUser = dbToApiUser(updatedUser);
      res.status(200).json({
        id: updatedDbUser.id,
      ...
      res.status(200).json(dbToApiUser(updatedUser));
    }
...
/* =========================================================
   Endpoints especificos de Working Hours
========================================================= */
exports.getUserWorkingHours = async (req, res) => {
  ...
  try {
    ...
    return res.status(200).json(user.working_hours || null);
  } catch (e) {
...
exports.getNextAvailableStylist = async (req, res) => {
  ...
  try {
    ...
    const tenantWH = tenant?.working_hours || null;
    ...
    const stylists = users.map(dbToApiUser);
...
/* =========================================================
   Buscar usuario por telefono (mismo tenant) - REQUIERE AUTH
========================================================= */
exports.getUserByPhone = async (req, res) => {
  ...
  if (!phoneNumber) {
    return res.status(400).json({ error: 'Numero de telefono no proporcionado.' });
  }
  if (!tenant_id) {
  ...
  try {
    ...
    if (!user) {
      return res.status(404).json({ message: 'Usuario no encontrado con ese numero de telefono.' });
    }
    return res.status(200).json(dbToApiUser(user));
  } catch (error) {
    console.error('Error al buscar usuario por telefono:', error);
    return res.status(500).json({ error: 'Error interno del servidor' });
  }
};
...
exports.getTenantClientsWithRecentServices = async (req, res) => {
  ...
  const sql = `
    ...
    FROM users u
    WHERE u.tenant_id = $1::uuid AND u.role_id = $2
    ORDER BY u.first_name, u.last_name;
  `;

  try {
    const rows = await prisma.$queryRawUnsafe(sql, tenantId, role_id_cliente);
...
/* ==================================================================
   ENDPOINTS PUBLICOS PARA WHATSAPP (SIN AUTENTICACION)
================================================================== */
  ...
  try {
    ...
    if (!user) {
      return res.status(404).json({
        message: 'Cliente no encontrado con ese numero de telefono.'
      });
    }

    const mapped = dbToApiUser(user);
    ...
    console.error('Error al buscar cliente por telefono (publico):', error);
  ...
  try {
    ...
    if (existingClient) {
      return res.status(409).json({
        error: 'Cliente ya existe con ese telefono o email',
    ...
    const newClient = dbToApiUser(newUser);
    ...
    if (error.code === 'P2002') {
      return res.status(409).json({
        error: 'El correo electronico o telefono ya esta registrado.'
      });
    }
    return res.status(500).json({ error: 'Error interno del servidor' });
  }
