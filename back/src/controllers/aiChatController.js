...
/**
 * Obtener la API Key de OpenAI (global, con cache)
 */
async function getOpenAIKey(_tenantId) {
    return getGlobalOpenAIKey();
}
    ...
    try {
        switch (functionName) {
            ...
            case 'listar_estilistas': {
                const result = await db.query(
                    `SELECT id, first_name, last_name
                     FROM users
                     WHERE tenant_id = $1::uuid AND role_id = 3 AND COALESCE(NULLIF(status,''),'active') = 'active'
                     ORDER BY first_name`,
                    [tenantId]
                );
            ...
            case 'agendar_cita': {
                ...
                try { getIO().to(`tenant:${tenantId}`).emit('appointment:created', { ...appointment, createdVia: 'ai_chat' }); } catch(e) {}
    ...
    try {
        ...
        if (!apiKey) {
            return res.status(400).json({
                error: 'El servicio de IA no está disponible en este momento. Contacta al administrador.'
            });
        }
        ...
        // Track token usage
        if (openaiData.usage) {
            trackUsage(tenantId, 'chat', 'gpt-4o-mini', openaiData.usage.prompt_tokens, openaiData.usage.completion_tokens, openaiData.usage.total_tokens);
        }
