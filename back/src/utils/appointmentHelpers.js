...
function isWithinRanges(dateStr, ranges, startDateTime, endDateTime) {
  ...
  // Convertir a hora local (Colombia) antes de comparar — .getHours() devuelve UTC
  const startLocal = formatInTimeZone(startDateTime, TIME_ZONE, 'HH:mm');
  const endLocal = formatInTimeZone(endDateTime, TIME_ZONE, 'HH:mm');
  const startMin = timeToMin(startLocal);
  const endMin = timeToMin(endLocal);
