    ...
    useEffect(() => {
        const fetchPrerequisites = async () => {
            ...
            try {
                ...
                const [productsRes, stylistsRes, tenantRes] = await Promise.all([productsPromise, stylistsPromise, tenantSettingsPromise]);
                ...
                // Get tip_salon_percent from first business (current tenant)
                            ...
                            <CardBody>
                                ...
                                <div>
                                    ...
                                    <Card className="p-3 border shadow-none"><Row className="gy-3">
                                        ...
                                        <Col md={12}><hr className="my-2" /><h6 className="mb-2">Propina (opcional)</h6><Row className="align-items-end"><Col md={4}><Label className="form-label">Monto propina</Label><CurrencyInput className="form-control" placeholder="$ 0" value={tipText} onValueChange={(value) => setTipText(value || "")} prefix="$ " groupSeparator="." decimalSeparator="," /></Col>{(Number(tipText) || 0) > 0 && (<Col md={8}><div className="text-muted small mt-2"><span className="me-3">Salón ({tipSalonPercent}%): <strong>{formatterCOP.format(Math.round((Number(tipText) || 0) * tipSalonPercent / 100))}</strong></span><span>Estilista ({100 - tipSalonPercent}%): <strong>{formatterCOP.format(Math.round((Number(tipText) || 0) * (1 - tipSalonPercent / 100)))}</strong></span></div></Col>)}</Row></Col>
                                    </Row></Card>
                                </div>
                            </CardBody>
                        </Card>
